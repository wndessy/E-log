...
$sql = "SELECT * FROM  $table1 WHERE (scm_corective_coment!='No'|scm_preventive_coment!='No')
			AND Target_center='$Center' ORDER BY report_no LIMIT $start, $per_page";
$rsd = mysql_query($sql);
?>
	...
	<table width="100%"  border="0" class="bodytable">
			 ...
			 <div class="formheader"> Unsatisfactory Reports Summery</div>		  
				...
				<tr>
				 ...
				 <?php 
				 //$result = mysql_query("SELECT * FROM  $table1 WHERE (scm_corective_coment!='No'|scm_preventive_coment!='No')
			//AND Target_center='$Center'") or die(mysql_error());
